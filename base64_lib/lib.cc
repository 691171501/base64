...
int decode_file_base64(std::string path_in, std::string path_out)
{
    ...
    {
        auto file_input = std::fopen(path_file.c_str(), "ab+");
        auto file_output= std::fopen(path_out.c_str(), "ab+");
...
int encode_file_base64_string(std::string path_in,  char* content)
{
    std::string path_file = path_in;
    ...
    if (is_readable(path_file))
    {
        ...
        if (file_input != NULL)
        {
            ...
            fclose(file_input);
            //fclose(file_output);
            std::strncpy(content, str_encode_buff.c_str(), strlen(str_encode_buff.c_str()) + 1);
...
int filetoBase64(char* filePath, char* outPut){
    return encode_file_base64(filePath, outPut);
}
int base64ToFile(char* filePath, char* outPut){
    return decode_file_base64(filePath, outPut);
}
int filetoBase64ToString(char* filePath,  char* content){
    return encode_file_base64_string(filePath,  content);
}
int base64ContentToFile(char* content, char* outPut) {
